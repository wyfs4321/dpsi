...
public class MqRpmtDpsiClient<T> extends AbstractDpsiClient<T> {
     ...
     * PSI-CA CDP
     */
    private final UnboundIntegralCdp psicaCdp;
    /**
     ...
     * PSD-CA CDP
     */
    private final UnboundIntegralCdp psdcaCdp;
    /**
     * element hash
     */
    ...
    public MqRpmtDpsiClient(Rpc clientRpc, Party serverParty, MqRpmtDpsiConfig config) {
        ...
        psicaCdp = UnboundIntegralCdpFactory.createInstance(config.getPsicaCdpConfig());
        maxPsdcaDummySize = config.getMaxPsdcaDummySize();
        psdcaCdp = UnboundIntegralCdpFactory.createInstance(config.getPsdcaCdpConfig());
    ...
    public void init(int maxClientElementSize, int maxServerElementSize) throws MpcAbortException {
        ...
        mqRpmtServer.init(
            ...
            maxServerElementSize + maxPsicaDummySize + maxPsdcaDummySize
        );
        stopWatch.stop();
    ...
    public Map<ByteBuffer, T> createDummyClientElementSet() {
        int psicaDummySize = Math.abs(psicaCdp.randomize(0));
        int psdcaDummySIze = Math.abs(psdcaCdp.randomize(0));
