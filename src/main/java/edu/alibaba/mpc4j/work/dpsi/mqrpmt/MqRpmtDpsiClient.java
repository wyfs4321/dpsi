...
public class MqRpmtDpsiClient<T> extends AbstractDpsiClient<T> {
     ...
     * PSI-CA CDP
     */
    private final BoundIntegralCdp psicaCdp;
    /**
     ...
     * PSD-CA CDP
     */
    private final BoundIntegralCdp psdcaCdp;
    /**
     * element hash
     */
    ...
    public MqRpmtDpsiClient(Rpc clientRpc, Party serverParty, MqRpmtDpsiConfig config) {
        ...
        psicaCdp = BoundIntegralCdpFactory.createInstance(config.getPsicaCdpConfig());
        maxPsdcaDummySize = config.getMaxPsdcaDummySize();
        psdcaCdp = BoundIntegralCdpFactory.createInstance(config.getPsdcaCdpConfig());
    ...
    public Map<ByteBuffer, T> createDummyClientElementSet() {
        int psicaDummySize = psicaCdp.randomize(0);
        int psdcaDummySize = psdcaCdp.randomize(0);
